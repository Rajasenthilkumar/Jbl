...
const IndividualStep2: FC<Props> = ({ setStep }) => {
  const { control, trigger, setValue } = useFormContext<IndividualSchema>();
  ...
  const onNext = async () => {
    ...
    if (isValid) {
      setStep(IndividualEntitySteps.STEP3);
    }
  };

  const onPrevious = () => {
    setStep(IndividualEntitySteps.STEP1);
  };
